...
const AuthState = (props) => {
  ...
  const ActivateAccount = async (formData) => {
    ...
    try {
      const res = await axios.post('/api/account-activation',
        formData,
        config
      );
      dispatch({
  ...
  const UpdateUser = async (formData) => {
    ...
    if (window !== 'undefined') {
      if (getCookie('token')) {
        ...
        try {
          const response = await axios.put('/api/user', formData, config);
  ...
  const UpdateAdmin = async (formData) => {
    ...
    if (window !== 'undefined') {
      if (getCookie('token')) {
        ...
        try {
          const response = await axios.put('/api/admin', formData, config );
